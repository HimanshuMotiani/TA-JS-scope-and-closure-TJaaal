...
function loop(startValue,testFn,updateFn,bodyFn) {
  ...
  for(let currentValue = startValue;testFn(currentValue);currentValue = updateFn(currentValue)){
    bodyFn(currentValue);
  }
}
...
// 2. The function `reduce` takes an array and reduces the elements to a single value.
// For example it can sum all the numbers, multiply them, or any operation that you can put into a function.

// Here's how it works. The function has an "accumulator value" which starts as the `initialValue` and 
// accumulates the output of each loop. The array is iterated over,
